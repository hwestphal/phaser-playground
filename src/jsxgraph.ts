...
//  export namespace TSX {
    /** crude  XSS filter (also replaces standard symbols with 'math' equivalents. */
        function xssFilter(str: String): string {
            let newStr = ''
            for (let i = 0; i < str.length; i++) {
                let danger = str.charAt(i)

                if (danger == '<') {            // left angle
                    newStr += '&#10216;'
                } else if (danger == '>') {     // right angle
                    newStr += '&#10217;'
                } else if (danger == '/') {     // slash
                    newStr += '&#8725;'
                } else if (danger == '"') {    // double quote
                    newStr += '&#698;'
                } else if (danger == '\'') {    // single quote
                    newStr += '&#8217;'
                } else {
                    newStr += danger            // have neutered the worst offenders
                }
            }
            return newStr;
        }
 ...
 /** Precision options for JSXGraph elements. This attributes takes either the value 'inherit' or an object of the form:precision: {touch: 30,mouse: 4,pen: 4 }In the first case, the global, JSXGraph-wide values of JXGraph.Options.precision are taken. */
  precision?: Object
 ...
 interface Curve3DAttributes extends GeometryElement3DAttributes {
 }
 ...
 interface VPointAttributes extends PointAttributes {
 }

 interface ClipAttributes extends MathAttributes {
 }
        type point = [Number|Function, Number | Function]
 ...
 interface CircleIface {
 z_ignore: Object,
 /** Two Points */
   protractor( centerPoint:Point|point, secondPoint:Point|point,attributes?:CircleAttributes):Circle
 /** Point and Radius */
   pointAndRadius( centerPoint:Point|point, p2:Point|point|Function|Line|line|Number,attributes?:CircleAttributes):Circle
 /** Point and Line, like Point and Radius, but takes the radius as the length of the line (which does not need to be near the circle).   */
   pointAndLine( centerPoint:Point|point, remotePoint:Point|point|Line|line|Number|Function|Circle,attributes?:CircleAttributes):Circle
 /** Point and Circle */
   pointAndCircle( p1:Point|point, p2:Circle,attributes?:CircleAttributes):Circle
}

 interface CurveIface {
 z_ignore: Object,
 /** Plot a set of points or a function from arrays X and Y */
   dataPlot( xArray:Number[]|Function, yArray:Number[]|Function,attributes?:CurveAttributes):Curve
 /** Apply a Transform to a Curve */
   transform( curve:Curve, transform:Transform,attributes?:CurveAttributes):Curve
}

 interface LineIface {
 z_ignore: Object,
 /** Defined by two points */
   twoPoints( p1:Point|point, p2:Point|point,attributes?:LineAttributes):Line
 /** Defined by solution to a*x + b*y == c */
   equation( a:Number|Function, b:Number|Function, c:Number|Function,attributes?:LineAttributes):Line
}

 interface AngleIface {
 z_ignore: Object,
 /** Angle defined by three points */
   angle3P( from:Point|point, around:Point|point, to:Point|point,attributes?:AngleAttributes):Angle
 /** Angle defined by two lines and two directions */
   angle2L( line1:Line|line, line2:Line|line, direction1:[Number,Number], direction2:[Number,Number],attributes?:AngleAttributes):Angle
}
 ...
 /** Two Points and Radius */
   pointsAndRadius( p1:Point|point, pointO:Point|point, radius:Number|Function,attributes?:EllipseAttributes):Ellipse
 ...
 /** Transform using a MAT3 */
   generic( x:Number|Function, y:Number|Function,attributes?:TransformAttributes):Transform
}

 interface VPointIface {
 z_ignore: Object,
 /** A Point and Transform */
   pointAndTransform( p:[Number|Function, Number | Function] | Point, t:Transform, label?:String,attributes?:VPointAttributes):VPoint
 /** A Point and multiple Transforms */
   pointAndTransformArray( p:Point|point, ttt:Transform[], label?:String,attributes?:VPointAttributes):VPoint
}
 ...
 z_ignore: {},
   /** Two Points */
   protractor( centerPoint:Point|point, secondPoint:Point|point,attributes: CircleAttributes ={}) : Circle {
 let newObject = ((this.z_ignore as any).board as any).create('Circle', (this.z_ignore as any).dereference([centerPoint, secondPoint, ]), attributes)
 return new Circle(newObject as Circle)
 },
   /** Point and Radius */
   pointAndRadius( centerPoint:Point|point, p2:Point|point|Function|Line|line|Number,attributes: CircleAttributes ={}) : Circle {
 let newObject = ((this.z_ignore as any).board as any).create('Circle', (this.z_ignore as any).dereference([centerPoint, p2, ]), attributes)
 return new Circle(newObject as Circle)
 },
   /** Point and Line, like Point and Radius, but takes the radius as the length of the line (which does not need to be near the circle).   */
   pointAndLine( centerPoint:Point|point, remotePoint:Point|point|Line|line|Number|Function|Circle,attributes: CircleAttributes ={}) : Circle {
  let newObject:any  // special case for circle with immediate segment eg:  circle(point,[[1,2],[3,4]]  )
                            if (Array.isArray(remotePoint) && Array.isArray(remotePoint[0] ) && Array.isArray(remotePoint[1] )) {
                                newObject = ((this.z_ignore as any).board as any).create(`circle`, (this.z_ignore as any).dereference([centerPoint, remotePoint[0] ,remotePoint[1]]), attributes)
                            } else {
                                newObject = ((this.z_ignore as any).board as any).create(`circle`, (this.z_ignore as any).dereference([centerPoint, remotePoint]), attributes)
                            }
 return new Circle(newObject as Circle)
 },
   /** Point and Circle */
   pointAndCircle( p1:Point|point, p2:Circle,attributes: CircleAttributes ={}) : Circle {
 let newObject = ((this.z_ignore as any).board as any).create('Circle', (this.z_ignore as any).dereference([p1, p2, ]), attributes)
 return new Circle(newObject as Circle)
 },
 }
 this.circle.z_ignore = this
 this.curve = {
 /** @protected */
 z_ignore: {},
   /** Plot a set of points or a function from arrays X and Y */
   dataPlot( xArray:Number[]|Function, yArray:Number[]|Function,attributes: CurveAttributes ={}) : Curve {
 let newObject = ((this.z_ignore as any).board as any).create('Curve', (this.z_ignore as any).dereference([xArray, yArray, ]), attributes)
 return new Curve(newObject as Curve)
 },
   /** Apply a Transform to a Curve */
   transform( curve:Curve, transform:Transform,attributes: CurveAttributes ={}) : Curve {
 let newObject = ((this.z_ignore as any).board as any).create('Curve', (this.z_ignore as any).dereference([curve, transform, ]), attributes)
 return new Curve(newObject as Curve)
 },
 }
 this.curve.z_ignore = this
 this.line = {
 /** @protected */
 z_ignore: {},
   /** Defined by two points */
   twoPoints( p1:Point|point, p2:Point|point,attributes: LineAttributes ={}) : Line {
 let newObject = ((this.z_ignore as any).board as any).create('Line', (this.z_ignore as any).dereference([p1, p2, ]), attributes)
 return new Line(newObject as Line)
 },
   /** Defined by solution to a*x + b*y == c */
   equation( a:Number|Function, b:Number|Function, c:Number|Function,attributes: LineAttributes ={}) : Line {
  if(a==0 && b==0){(this.z_ignore as any).print('Line has A==0 and B==0');return null as any as Line}

 let newObject = (this.z_ignore as any).create('line', (this.z_ignore as any).dereference([c,b,a]), attributes)
 return new Line(newObject as Line)
 },
 }
 this.line.z_ignore = this
 this.angle = {
 /** @protected */
 z_ignore: {},
   /** Angle defined by three points */
   angle3P( from:Point|point, around:Point|point, to:Point|point,attributes: AngleAttributes ={}) : Angle {
 let newObject = ((this.z_ignore as any).board as any).create('Angle', (this.z_ignore as any).dereference([from, around, to, ]), attributes)
 return new Angle(newObject as Angle)
 },
   /** Angle defined by two lines and two directions */
   angle2L( line1:Line|line, line2:Line|line, direction1:[Number,Number], direction2:[Number,Number],attributes: AngleAttributes ={}) : Angle {
 let newObject = ((this.z_ignore as any).board as any).create('Angle', (this.z_ignore as any).dereference([line1, line2, direction1, direction2, ]), attributes)
 return new Angle(newObject as Angle)
 },
 }
 this.angle.z_ignore = this
 this.conic = {
 /** @protected */
 z_ignore: {},
   /** Just as two (distinct) points determine a line, five points (no three collinear) determine a conic. */
   fivePoints( A:Point|point, B:Point|point, C:Point|point, D:Point|point, E:Point|point,attributes: ConicAttributes ={}) : Conic {
 let newObject = ((this.z_ignore as any).board as any).create('Conic', (this.z_ignore as any).dereference([A, B, C, D, E, ]), attributes)
 return new Conic(newObject as Conic)
 },
   /** Build a plane algebraic curve from six numbers that satisfies Ax^2 + Bxy + Cy^2 + Dx + Ey + F = 0, and A,B,C not all zero.  This might be a circle, ellipse, parabola, or hyperbola. */
   sixNumbers( A:Number, B:Number, C:Number, D:Number, E:Number, F:Number,attributes: ConicAttributes ={}) : Conic {
 let newObject = ((this.z_ignore as any).board as any).create('Conic', (this.z_ignore as any).dereference([A, B, C, D, E, F, ]), attributes)
 return new Conic(newObject as Conic)
 },
 }
 ...
 z_ignore: {},
   /** Three Points */
   threePoints( focalPoint1:Point|point, focalPoint2:Point|point, outerPoint:Point|point,attributes: EllipseAttributes ={}) : Ellipse {
 let newObject = ((this.z_ignore as any).board as any).create('Ellipse', (this.z_ignore as any).dereference([focalPoint1, focalPoint2, outerPoint, ]), attributes)
 return new Ellipse(newObject as Ellipse)
 },
   /** Two Points and Radius */
   pointsAndRadius( p1:Point|point, pointO:Point|point, radius:Number|Function,attributes: EllipseAttributes ={}) : Ellipse {
 let newObject = ((this.z_ignore as any).board as any).create('Ellipse', (this.z_ignore as any).dereference([p1, pointO, radius, ]), attributes)
 return new Ellipse(newObject as Ellipse)
 },
   /** Three Points, plus start and end. */
   ellipseArc( focalPoint1:Point|point, focalPoint2:Point|point, outerPoint:Point|point, startAngle:Number|Function, endAngle:Number|Function,attributes: EllipseAttributes ={}) : Ellipse {
 let newObject = ((this.z_ignore as any).board as any).create('Ellipse', (this.z_ignore as any).dereference([focalPoint1, focalPoint2, outerPoint, startAngle, endAngle, ]), attributes)
 return new Ellipse(newObject as Ellipse)
 },
 }
 this.ellipse.z_ignore = this
 this.transform = {
 /** @protected */
 z_ignore: {},
   /** Move a distance from a point */
   translate( x:Number|Function, y:Number|Function,attributes: TransformAttributes ={}) : Transform {
 let newObject = ((this.z_ignore as any).board as any).create('Transform', (this.z_ignore as any).dereference([x, y, ]), {type:'translate' })
 return new Transform(newObject as Transform)
 },
   /** Increase distance from a point by a factor */
   scale( x:number|Function, y:number|Function,attributes: TransformAttributes ={}) : Transform {
 let newObject = ((this.z_ignore as any).board as any).create('Transform', (this.z_ignore as any).dereference([x, y, ]), {type:'scale' })
 return new Transform(newObject as Transform)
 },
   /** Rotate by angle around a point */
   rotate( angle:Number|Function, point:Point|point,attributes: TransformAttributes ={}) : Transform {
 let newObject = ((this.z_ignore as any).board as any).create('Transform', (this.z_ignore as any).dereference([angle, point, ]), {type:'rotate' })
 return new Transform(newObject as Transform)
 },
   /** Reflect around a line */
   reflect( x:Number|Function, y:Number|Function,attributes: TransformAttributes ={}) : Transform {
 let newObject = ((this.z_ignore as any).board as any).create('Transform', (this.z_ignore as any).dereference([x, y, ]), {type:'reflect' })
 return new Transform(newObject as Transform)
 },
   /** Move proportionally to distance */
   shear( x:Number|Function, y:Number|Function,attributes: TransformAttributes ={}) : Transform {
 let newObject = ((this.z_ignore as any).board as any).create('Transform', (this.z_ignore as any).dereference([x, y, ]), {type:'shear' })
 return new Transform(newObject as Transform)
 },
   /** Transform using a MAT3 */
   generic( x:Number|Function, y:Number|Function,attributes: TransformAttributes ={}) : Transform {
 let newObject = ((this.z_ignore as any).board as any).create('Transform', (this.z_ignore as any).dereference([x, y, ]), {type:'generic' })
 return new Transform(newObject as Transform)
 },
 }
 ...
 if(!('strokeColor' in attributes)) attributes['strokeColor'] = 'red'

 if(!('fillColor' in attributes)) attributes['strokeColor'] = 'blue'

 let newObject = ((this.z_ignore as any).board as any).create('Point', (this.z_ignore as any).dereference([p,t, ]), attributes)
 return new VPoint(newObject as VPoint)
 },
   /** A Point and multiple Transforms */
   pointAndTransformArray( p:Point|point, ttt:Transform[], label:String=``,attributes: VPointAttributes ={}) : VPoint {
 attributes['name'] = label;
 ...
 let newObject = ((this.z_ignore as any).board as any).create('Point', (this.z_ignore as any).dereference([p,ttt,]), attributes)
 return new VPoint(newObject as VPoint)
 },
 }
 this.vPoint.z_ignore = this
}
            ...
            public print(...args: any[]) {
                let bbox = (this.board as any).getBoundingBox()
                let left = bbox[0] // align x to left border
                let height = (bbox[1] - bbox[3]) / 20
                let top = bbox[1] - (2 * height) - this.printLineNumber  // align y to top border
                ...
                let stringText = ''
                args.forEach((argn) => {
                    // stringText += (stringText == '') ? '' : ', '    // separator
                    stringText = helper(stringText, argn)
                });

                (this.board as any).create('text', this.dereference([left, top, xssFilter(stringText)]), { fontSize: 10, strokeColor: 'blue' })
                this.printLineNumber += height
            }
...
chart(f:Number[], attributes: ChartAttributes ={} ):Chart{
 let newObject = (this.board as any).create('Chart', this.dereference([f,]), attributes)
...
group(pointArray:Point[], attributes: GroupAttributes ={} ):Group{
 let newObject = (this.board as any).create('Group', this.dereference([pointArray,].flat()), attributes)
...
image(url:String, lowerLeft:point, widthHeight:[Number,Number], attributes: ImageAttributes ={} ):Image{
 let newObject = (this.board as any).create('Image', this.dereference([url,lowerLeft,widthHeight,]), attributes)
 return new Image(newObject as Image)
}


 /** Specify XY location  This element is used to provide a constructor for a general point. A free point is created if the given parent elements are all numbers and the property fixed is not set or set to false. If one or more parent elements is not a number but a string containing a GEONExT constraint or a function the point will be considered as constrained). That means that the user won't be able to change the point's position directly.*/
point(x:Number|Function, y:Number | Function, label:String=``, attributes: PointAttributes ={} ):Point{
 attributes['name'] = label;

 let newObject = (this.board as any).create('Point', [x,y], attributes)
...
polygon(pointArray:Point[], attributes: PolygonAttributes ={} ):Polygon{
 let newObject = (this.board as any).create('Polygon', this.dereference([pointArray,].flat()), attributes)
...
view3D(x:Number=-13, y:Number=-10, w:Number=20, h:Number=20, x1:Number=-5, x2:Number=5, y1:Number=-5, y2:Number=5, z1:Number=-5, z2:Number=5, attributes: View3DAttributes={
                         // Main axes
                         axesPosition: 'center',
 ...
 let newObject = (this.board as any).create('view3D',[ [x,y],[w,h],[[x1,x2],[y1,y2],[z1,z2]]], attributes)
...
arrow(p1:Point|point, p2:Point|point, attributes: ArrowAttributes ={} ):Arrow{
 let newObject = (this.board as any).create('Arrow', this.dereference([p1,p2,]), attributes)
...
axis(p1:Point|point, p2:Point|point, attributes: AxisAttributes ={} ):Axis{
 let newObject = (this.board as any).create('Axis', this.dereference([p1,p2,]), attributes)
...
bisectorlines(l1:Line, l2:Line, attributes: BisectorlinesAttributes ={} ):Bisectorlines{
 let newObject = (this.board as any).create('Bisectorlines', this.dereference([l1,l2,]), attributes)
...
button(x:Number|Function, y:Number|Function, label:String, handler:Function, attributes: ButtonAttributes ={} ):Button{
 let newObject = (this.board as any).create('button', [x,y,xssFilter(label)], attributes)
...
circumcenter(p1:Point|point, p2:Point|point, p3:Point|point, label:String=``, attributes: CircumcenterAttributes ={} ):Circumcenter{
 attributes['name'] = label;

 let newObject = (this.board as any).create('Circumcenter', [p1,p2,p3], attributes)
...
circumcircle(p1:Point|point, p2:Point|point, p3:Point|point, attributes: CircumcircleAttributes ={} ):Circumcircle{
 let newObject = (this.board as any).create('Circumcircle', this.dereference([p1,p2,p3,]), attributes)
...
circumcircleArc(p1:Point|point, p2:Point|point, p3:Point|point, attributes: CircumcircleArcAttributes ={} ):CircumcircleArc{
 let newObject = (this.board as any).create('CircumcircleArc', this.dereference([p1,p2,p3,]), attributes)
...
circumcircleSector(p1:Point|point, p2:Point|point, p3:Point|point, attributes: CircumcircleSectorAttributes ={} ):CircumcircleSector{
 let newObject = (this.board as any).create('CircumcircleSector', this.dereference([p1,p2,p3,]), attributes)
...
comb(p1:Point|point, p2:Point|point, attributes: CombAttributes ={} ):Comb{
 let newObject = (this.board as any).create('Comb', this.dereference([p1,p2,]), attributes)
...
curveDifference(curve1:GeometryElement, curve2:GeometryElement, attributes: CurveDifferenceAttributes ={} ):CurveDifference{
 let newObject = (this.board as any).create('CurveDifference', this.dereference([curve1,curve2,]), attributes)
...
curveIntersection(curve1:GeometryElement, curve2:GeometryElement, attributes: CurveIntersectionAttributes ={} ):CurveIntersection{
 let newObject = (this.board as any).create('CurveIntersection', this.dereference([curve1,curve2,]), attributes)
...
curveUnion(curve1:GeometryElement, curve2:GeometryElement, attributes: CurveUnionAttributes ={} ):CurveUnion{
 let newObject = (this.board as any).create('CurveUnion', this.dereference([curve1,curve2,]), attributes)
...
functiongraph(funct:Function, leftBorder?:Number, rightBorder?:Number, attributes: FunctiongraphAttributes ={} ):Functiongraph{
 let newObject = (this.board as any).create('Functiongraph', this.dereference([funct,leftBorder,rightBorder,]), attributes)
 return new Functiongraph(newObject as Functiongraph)
}


 /** A GeometryElement like Line, Circle, or Curve, and optionally a starting point defined by X,Y  This element is used to provide a constructor for a glider point.*/
glider(hostElement:GeometryElement, x:Number=0, y:Number=0, attributes: GliderAttributes ={} ):Glider{
 let newObject = (this.board as any).create('glider', this.dereference([x,y,hostElement])) as GeometryElement
...
grid(showGrid:Boolean=true, attributes: GridAttributes ={} ):Grid{
 let newObject = (this.board as any).create('Grid', this.dereference([showGrid,]), attributes)
...
hatch(line:Line|line, numberHatches:Number, attributes: HatchAttributes ={} ):Hatch{
 let newObject = (this.board as any).create('Hatch', this.dereference([line,numberHatches,]), attributes)
...
inequality(boundaryLine:Line|line|Curve, attributes: InequalityAttributes ={} ):Inequality{
 let newObject = (this.board as any).create('Inequality', this.dereference([boundaryLine,]), attributes)
...
input(x:Number|Function, y:Number|Function, prompt:String, initial:String, attributes: InputAttributes ={} ):Input{
 let newObject = (this.board as any).create('Input', this.dereference([x,y,prompt,initial,]), attributes)
...
intersection(element1:Line|Circle, element2:Line|Circle, positiveNegativeRoot:Number=0, label:String=``, attributes: IntersectionAttributes ={} ):Point{
 attributes['name'] = label;

 let newObject = (this.board as any).create('intersection', this.dereference([element1,element2,positiveNegativeRoot]), attributes)
 return new Point(newObject as Point)
}


 /**   The midpoint element constructs a point in the middle of two given points.*/
midpoint(p1:Point|point, p2:Point|point, attributes: MidpointAttributes ={} ):Midpoint{
 let newObject = (this.board as any).create('Midpoint', this.dereference([p1,p2,]), attributes)
...
parabola(focalPoint:Point|point, line:Line|line, attributes: ParabolaAttributes ={} ):Parabola{
 let newObject = (this.board as any).create('Parabola', this.dereference([focalPoint,line,]), attributes)
...
segment(P1:Point|point, P2:Point|point, attributes: SegmentAttributes ={} ):Segment{
 let newObject = (this.board as any).create('Segment', this.dereference([P1,P2,]), attributes)
...
regularPolygon(P1:Point|point, P2:Point|point, nVertices:Number, attributes: RegularPolygonAttributes ={} ):RegularPolygon{
 let newObject = (this.board as any).create('RegularPolygon', this.dereference([P1,P2,nVertices,]), attributes)
 return new RegularPolygon(newObject as RegularPolygon)
}


 /**   A slider can be used to choose values from a given range of numbers.*/
slider(P1:Point|point, P2:Point|point, minimumMaximumSteps:[number,number,number], attributes: SliderAttributes ={} ):Slider{
 let newObject = (this.board as any).create('Slider', this.dereference([P1,P2,minimumMaximumSteps,]), attributes)
...
tangent(glider:Glider, attributes: TangentAttributes ={} ):Tangent{
 let newObject = (this.board as any).create('Tangent', this.dereference([glider,]), attributes)
...
tapemeasure(P1:Point|point, P2:Point|point, attributes: TapemeasureAttributes ={} ):Tapemeasure{
 let newObject = (this.board as any).create('Tapemeasure', this.dereference([P1,P2,]), attributes)
 ...
 /**  */
 public get ancestors():Object {
  return (this.elValue as any).ancestors as Object
}

 /**  */
 public get baseElement():GeometryElement {
  return (this.elValue as any).baseElement as GeometryElement
}
 ...
 /**  */
 public get childElements():Object {
  return (this.elValue as any).childElements as Object
}

 /**  */
 public get descendants():Object {
  return (this.elValue as any).descendants as Object
}

 /**  */
 public get dump():Boolean {
  return (this.elValue as any).dump as Boolean
}

 /**  */
 public get elementClass():Object {
  return (this.elValue as any).elementClass as Object
}
 ...
 public get hasLabel():Boolean {
  return (this.elValue as any).hasLabel as Boolean
}

 /**  */
 public get highlighted():Boolean {
  return (this.elValue as any).highlighted as Boolean
}

 /**  */
 public get id():String {
  return (this.elValue as any).id as String
}

 /**  */
 public get inherits():Object {
  return (this.elValue as any).inherits as Object
}
 ...
 /**  */
 public get lastDragTime():Date {
  return (this.elValue as any).lastDragTime as Date
}

 /**  */
 public get methodMap():Object {
  return (this.elValue as any).methodMap as Object
}

 /**  */
 public get mouseover():Boolean {
  return (this.elValue as any).mouseover as Boolean
}
 ...
 public get needsUpdate():Boolean {
  return (this.elValue as any).needsUpdate as Boolean
}

 /**  */
 public get notExistingParents():Object {
  return (this.elValue as any).notExistingParents as Object
}

 /**  */
 public get numTraces():Number {
  return (this.elValue as any).numTraces as Number
}

 /**  */
 public get parents():Object {
  return (this.elValue as any).parents as Object
}

 /**  */
 public get quadraticform():Number[] {
  return (this.elValue as any).quadraticform as Number[]
}

 /**  */
 public get rendNode():Object {
  return (this.elValue as any).rendNode as Object
}

 /**  */
 public get stdform():Number[] {
  return (this.elValue as any).stdform as Number[]
}

 /**  */
 public get subs():Object {
  return (this.elValue as any).subs as Object
}

 /**  */
 public get symbolic():Object {
  return (this.elValue as any).symbolic as Object
}

 /**  */
 public get traces():Object {
  return (this.elValue as any).traces as Object
}

 /**  */
 public get transformations():Number[] {
  return (this.elValue as any).transformations as Number[]
}

 /**  */
 public get type():Number {
  return (this.elValue as any).type as Number
}

 /**  */
 public get visProp():Object {
  return (this.elValue as any).visProp as Object
}

 /**  */
 public get visPropCalc():Object {
  return (this.elValue as any).visPropCalc as Object
}
 ...
 export class Curve3D extends GeometryElement3D {
 ...
 public get az_slide():Number[] {
  return (this.elValue as any).az_slide as Number[]
}

 /**  */
 public get bbox3D():Number[] {
  return (this.elValue as any).bbox3D as Number[]
}
 ...
 public get el_slide():Object {
  return (this.elValue as any).el_slide as Object
}

 /**  */
 public get elementsByName():Object {
  return (this.elValue as any).elementsByName as Object
}

 /**  */
 public get llftCorner():Number[] {
  return (this.elValue as any).llftCorner as Number[]
}
 ...
 public get objects():Object {
  return (this.elValue as any).objects as Object
}

 /**  */
 public get r():Number {
  return (this.elValue as any).r as Number
}
 ...
 export class ParametricSurface3D extends Curve {
 constructor(elValues: Curve){
   super(elValues)
}
}
 ...
 /**  */
 applyOnce(): Transform {
  return (this.elValue as any).applyOnce() as Transform
}

 /**  */
 bindTo(element:GeometryElement): Transform {
  return (this.elValue as any).bindTo(JSXGraph.dereference(element)) as Transform
}

 /**  */
 melt(): Transform {
  return (this.elValue as any).melt() as Transform
}
 ...
 setProperty(): Transform {
  return (this.elValue as any).setProperty() as Transform
}
}

 export class VPoint extends Point {
 constructor(elValues: Point){
   super(elValues)
}
}

 export class Clip extends Math {
 constructor(elValues: Math){
   super();        // math classes are static
}

 /**  */
 difference(curve2:GeometryElement,curve1:GeometryElement): [number[],number[]] {
  return (window as any).JXG.Math.Clip.difference(curve2,curve1,Math.board) as [number[],number[]]
}

 /**  */
 intersection(curve2:GeometryElement,curve1:GeometryElement): [number[],number[]] {
  return (window as any).JXG.Math.Clip.intersection(curve2,curve1,Math.board) as [number[],number[]]
}

 /**  */
 union(curve2:GeometryElement,curve1:GeometryElement): [number[],number[]] {
  return (window as any).JXG.Math.Clip.union(curve2,curve1,Math.board) as [number[],number[]]
}
}